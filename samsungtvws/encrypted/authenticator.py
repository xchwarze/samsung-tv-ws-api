...
class SamsungEncrypted:
    ...
    def _encrypt_parameter_data_with_aes(self, data: bytes) -> bytes:
        iv = b"\x00" * self.BLOCK_SIZE
        output = b""
        for num in range(0, 128, 16):
            cipher = Cipher(algorithms.AES(bytes.fromhex(self.WB_KEY)), modes.CBC(iv))
            encryptor: CipherContext = cipher.encryptor()
            output += encryptor.update(data[num : num + 16]) + encryptor.finalize()

        return output

    def _decrypt_parameter_data_with_aes(self, data: bytes) -> bytes:
        iv = b"\x00" * self.BLOCK_SIZE
        output = b""
        for num in range(0, 128, 16):
            cipher = Cipher(algorithms.AES(bytes.fromhex(self.WB_KEY)), modes.CBC(iv))
    ...
    def _apply_samy_go_key_transform(self, data: bytes) -> bytes:
    ...
    def parse_client_hello(
        ...
        pEncGx = self._decrypt_parameter_data_with_aes(pEncWBGx)
        ...
        bnPrime = int(self.PRIME, 16)
        bnPrivateKey = int(self.PRIVATE_KEY, 16)
        ...
        LOGGER.info("Pin OK")
        flagPos = userIdLen + USER_ID_POS + GX_SIZE + self.SHA_DIGEST_LENGTH
    ...
    def generate_server_acknowledge(self, skprime: bytes) -> str:
        ...
        return self.ACK_HEADER_START + skprime_hash.hex().upper() + self.ACK_HEADER_END

    def parse_client_acknowledge(self, client_ack: str, skprime: bytes) -> bool:
        sha1 = hashlib.sha1()
...
class SamsungTVEncryptedWSAsyncAuthenticator:
    ...
    async def _second_step_of_pairing(self, pin: str) -> Optional[Dict[str, bytes]]:
        hello_output = SamsungEncrypted.generate_server_hello(self.USER_ID, pin)
        if not hello_output:
            return None
        ...
        return SamsungEncrypted.parse_client_hello(
    ...
    async def _acknowledge_exchange(self) -> str:
        ...
        assert self._sk_prime
        if not SamsungEncrypted.parse_client_acknowledge(client_ack, self._sk_prime):
