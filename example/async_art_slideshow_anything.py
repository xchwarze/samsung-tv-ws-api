...
def parseargs():
    ...
    parser = argparse.ArgumentParser(
        description="Async Slideshow Any art on Samsung TV."
    )
    parser.add_argument(
        "ip",
        action="store",
    ...
    parser.add_argument(
        ...
        type=str,
        default="./slideshow",
        help="folder to store images in (default: %(default)s))",
    )
    parser.add_argument(
        "-c",
        "--check",
    ...
    parser.add_argument(
        ...
        type=float,
        default=2,
        help="random update period (mins) 0.25 minnimum (default: %(default)s))",
    )
    parser.add_argument(
        "-D",
        "--debug",
...
class slideshow:
    category = Enum(
        value="categories",
        names=[
            ("MY_PHOTOS", ("my_photos", "MY-C0002", None, set())),
    ...
    def __init__(self, ip, folder, period=60, random_update=1440):
        ...
        self.category.MY_PHOTOS.dir = os.path.join(
            folder, self.category.MY_PHOTOS.dir_name
        )
        self.category.FAVOURITES.dir = os.path.join(
            folder, self.category.FAVOURITES.dir_name
        )
        self.api_version = 1
        ...
        self.log.info(
            "check thumbnails {}, slideshow rotation every: {}".format(
                f"every {self.period}s" if self.period else "once",
                datetime.timedelta(seconds=self.random_update),
            )
        )
        try:
    ...
    async def initialize(self):
        ...
        if not self.get_files(self.category.SLIDESHOW):
            ...
            for file in self.get_files(self.category.FAVOURITES):
                shutil.copy2(
                    os.path.join(self.category.FAVOURITES.dir, file),
                    self.category.SLIDESHOW.dir,
                )
        await self.set_start()

    async def set_start(self):
        content_id = (await self.tv.get_current()).get("content_id")
        self.log.debug(f"got current artwork: {content_id}")
        self.start = max(
            time.time() - self.random_update * 60,
    ...
    def remove_files(self, cat):
        ...
        remove_files = [
            ...
            for f in files
            if self.get_content_ids(f) in my_files_remove
        ]
        self.log.debug(f"photos to remove: {my_files_remove}")
        self.log.info(f"removing: {remove_files}")
        for path in remove_files:
            if os.path.exists(path):
                self.log.debug(f"deleting file: {path}")
                os.unlink(path)
            else:
                self.log.warning(f"cannot remove {path} as it does not exist")
    ...
    async def do_random_update(self):
        if time.time() - self.start > self.random_update:
            self.log.info(
                f"doing random update, after {datetime.timedelta(seconds = self.random_update)}"
            )
            self.start = time.time()
            await self.download_thmbnails()
            slideshow_files = list(
                self.get_content_ids(self.get_files(self.category.SLIDESHOW))
            )
            if slideshow_files:
        ...
        try:
            for filename, data in thumbnails.items():
                ...
                if not os.path.exists(path):
                    self.log.info(f"writing {path}")
                    with open(path, "wb") as f:
                        f.write(data)
        except Exception as e:
            self.log.error(f"error writing file: {os.path.join(folder, filename)}, {e}")
    ...
    def get_countdown(self):
        return datetime.timedelta(
            seconds=max(0, (self.random_update - (time.time() - self.start)))
        )
        ...
        while True:
            try:
                ...
                if await self.tv.is_artmode():
                    self.log.info(f"time to next rotation: {self.get_countdown()}")
                ...
                self.log.warning(f"error in select_artwork: {e}")
                if self.debug:
